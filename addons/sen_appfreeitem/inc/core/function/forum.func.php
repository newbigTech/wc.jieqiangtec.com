...
function send_template($uid,$code,$other = array()){
	...
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_msg_template')." WHERE uniacid = :uniacid AND type = :type ";
...
function getTopicById($id){
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_topics')." WHERE id = :id";
	$params = array(':id'=>$id);
...
function replace_em($str){
	$str = preg_replace('/\[em_(.*?)\]/', '<img src="../addons/intelligent_kindergarten/template/mobile/default/lib/qqface/arclist/$1.gif">', $str);
	return $str;
}
...
function getnavs(){
	global $_W;
	$table = 'intelligent_kindergarten_navs';
...
function insert_home_message($fromopenid,$toopenid,$tid , $type , $log){
	...
	$insert['tid'] = $tid;
	$insert['content'] = $log;
	pdo_insert('intelligent_kindergarten_home_message',$insert);
...
function getAuthSet(){
	global $_W;
	$module = 'intelligent_kindergarten';
...
function getCredit($tid){
	global $_W;
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_share')." WHERE tid = :tid AND uniacid = :uniacid";
...
function getMyTopicsAll(){
	global $_W;
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_topics')." WHERE uniacid = :uniacid AND uid = :uid ORDER BY createtime DESC";
	...
	foreach ($topics as $topic){
		...
		$sql = "SELECT name FROM ".tablename('intelligent_kindergarten_threadclass')." WHERE typeid = :typeid";
		$params = array(':typeid'=>$topic['fid']);
		$topic['class'] = pdo_fetchcolumn($sql,$params);
		
		$sql = "SELECT COUNT(*) FROM ".tablename('intelligent_kindergarten_topic_share')." WHERE tid = :tid";
		$params = array(':tid'=>$topic['id']);
		$sharenum = pdo_fetchcolumn($sql,$params);
		
		$sql = "SELECT COUNT(*) FROM ".tablename('intelligent_kindergarten_topic_replie')." WHERE tid = :tid";
		$params = array(':tid'=>$topic['id']);
		$replynum = pdo_fetchcolumn($sql,$params);
		
		$sql = "SELECT COUNT(*) FROM ".tablename('intelligent_kindergarten_topic_like')." WHERE tid = :tid";
		$params = array(':tid'=>$topic['id']);
		$likenum = pdo_fetchcolumn($sql,$params);
		
		$sql = "SELECT SUM(fee) FROM ".tablename('intelligent_kindergarten_begging')." WHERE ttid = :ttid AND status = 1";
...
function getMyTopics(){
	global $_W;
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_topics')." WHERE uniacid = :uniacid AND uid = :uid ORDER BY rand() DESC limit 0,5";
	$params = array(':uniacid'=>$_W['uniacid'],':uid'=>$_W['member']['uid']);
	$topics = pdo_fetchall($sql,$params);
	foreach ($topics as $topic){
		$topic['thumb'] = iunserializer($topic['thumb']);
...
function getTotalMyTopics(){
	global $_W;
	$sql = "SELECT COUNT(*) FROM ".tablename('intelligent_kindergarten_topics')." WHERE uniacid = :uniacid AND uid = :uid";
	$params = array(':uniacid'=>$_W['uniacid'],':uid'=>$_W['member']['uid']);
...
function check_look($id){
	...
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_threadclass')." WHERE typeid = :id limit 1";
	$params = array(':id'=>$id);
...
function getSet(){
	global $_W;
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_set')." WHERE uniacid = :uniacid limit 1";
...
function updateSet($set){
	global $_W;
	$data = serialize($set);
	pdo_update('intelligent_kindergarten_set',array('set'=>$data),array('uniacid'=>$_W['uniacid']));
	return true;
}
...
function getAdv(){
	global $_W;
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_adv')." WHERE uniacid = :uniacid AND enabled = :enabled ORDER BY RAND() LIMIT 1";
	$params = array(':uniacid'=>$_W['uniacid'],':enabled'=>1);
...
function getCat(){
	...
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_threadclass')." WHERE uniacid = :uniacid AND fid = :fid ORDER BY displayorder DESC";
	$list = pdo_fetchall($sql,$params);
	foreach ($list as $li) {
...
function getTop(){
	global $_W;
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_topics')." WHERE uniacid = :uniacid AND tab = :tab ORDER BY rand() DESC limit 0,5";
...
function getJing($id){
	...
	$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_topics')." WHERE uniacid = :uniacid AND tab = :tab ORDER BY rand() DESC limit 0,3";
	$params = array(':uniacid'=>$_W['uniacid'],':tab'=>'jing');
	if(!empty($id)){
		$sql = "SELECT * FROM ".tablename('intelligent_kindergarten_topics')." WHERE uniacid = :uniacid AND tab = :tab AND fid = :fid ORDER BY rand() DESC limit 0,3";
