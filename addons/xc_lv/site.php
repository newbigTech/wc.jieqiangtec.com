...
class Xc_lvModuleSite extends WeModuleSite
{
        ...
        if ($operation == 'display') {
            if (!empty($_GPC['displayorder'])) {
                foreach ($_GPC['displayorder'] as $id => $displayorder) {
                    pdo_update('xc_lv_category', array(
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($id)) {
                    unset($data['parentid']);
                    pdo_update('xc_lv_category', $data, array(
                        'id' => $id
                    ));
                    load()->func('file');
            ...
            pdo_delete('xc_lv_category', array(
                'id' => $id,
                'parentid' => $id
            ), 'OR');
    ...
    public function doWebGoods()
    {
        ...
        if ($operation == 'post') {
            $id = intval($_GPC['id']);
            if (!empty($id)) {
                $item = pdo_fetch("SELECT * FROM " . tablename('xc_lv_goods') . " WHERE id = :id", array(
                    ':id' => $id
                ));
                if (empty($item)) {
                    message('抱歉，商品不存在或是已经删除！', '', 'error');
                }
                $allspecs = pdo_fetchall("SELECT * FROM " . TABLENAME('xc_lv_spec') . " WHERE goodsid=:id ORDER BY displayorder ASC", array(
                    ":id" => $id
                ));
                foreach ($allspecs as &$s) {
                    $s['items'] = pdo_fetchall("select * from " . tablename('xc_lv_spec_item') . " where specid=:specid order by displayorder asc", array(
                        ":specid" => $s['id']
                    ));
                }
                unset($s);
                $params   = pdo_fetchall("select * from " . tablename('xc_lv_goods_param') . " where goodsid=:id order by displayorder asc", array(
                    ':id' => $id
                ));
                ...
                $html    = "";
                $options = pdo_fetchall("select * from " . tablename('xc_lv_goods_option') . " where goodsid=:id order by id asc", array(
                    ':id' => $id
                ));
                $specs   = array();
                if (count($options) > 0) {
            ...
            if (checksubmit('submit')) {
                ...
                $birth     = $_GPC['birth'];
                $datime[0] = $_GPC['succlefttpl'];
                $datime[1] = $_GPC['succrighttpl'];
                array_pop($datime[0]);
                array_pop($datime[1]);
                ...
                $yinpin[0] = $_GPC['yinpinlefttpl'];
                $yinpin[1] = $_GPC['yinpinzhongtpl'];
                $yinpin[2] = $_GPC['yinpinrighttpl'];
                array_pop($yinpin[0]);
                array_pop($yinpin[1]);
                array_pop($yinpin[2]);
                $yinpin   = json_encode($yinpin);
                $zhi      = $_GPC['zhi'];
                $han      = $_GPC['han'];
                ...
                if (empty($id)) {
                    ...
                    pdo_update('xc_lv_goods', $data, array(
                        'id' => $id
                    ));
                }
                $totalstocks         = 0;
                ...
                for ($k = 0; $k < $len; $k++) {
                    ...
                    if (!is_numeric($get_param_id)) {
                        ...
                        pdo_update("xc_lv_goods_param", $a, array(
                            'id' => $get_param_id
                        ));
                ...
                for ($k = 0; $k < $len; $k++) {
                    ...
                    for ($n = 0; $n < $itemlen; $n++) {
                        ...
                        if (is_numeric($get_item_id)) {
                            pdo_update("xc_lv_spec_item", $d, array(
                                "id" => $get_item_id
                            ));
                            $item_id = $get_item_id;
                        } else {
                ...
                for ($k = 0; $k < $len; $k++) {
                    ...
                    if (empty($get_option_id)) {
                        ...
                        pdo_update("xc_lv_goods_option", $a, array(
                            'id' => $get_option_id
                        ));
            ...
            $id  = intval($_GPC['id']);
            $row = pdo_fetch("SELECT id, thumb FROM " . tablename('xc_lv_goods') . " WHERE id = :id", array(
                ':id' => $id
            ));
            if (empty($row)) {
            ...
            $id = intval($_GPC['id']);
            pdo_delete('xc_lv_product', array(
                'id' => $id
            ));
    ...
    public function doWebOrder()
    {
        ...
        if ($set['ispay'] == 1) {
            ...
            if ($operation == 'display') {
                ...
                if ($total > 0) {
                    ...
                    foreach ($list as &$value) {
                        ...
                        $value['dispatch']  = pdo_fetchcolumn("SELECT `dispatchname` FROM " . tablename('xc_lv_dispatch') . " WHERE id = :id", array(
                            ':id' => $value['dispatch']
                        ));
                ...
                $id = intval($_GPC['id']);
                global $_GPC, $_W;
                $item    = pdo_fetch("SELECT * FROM " . tablename('xc_lv_order') . " WHERE id = :id", array(
                    ':id' => $id
                ));
                ...
                if (checksubmit('confirmsend')) {
                    ...
                    $item = pdo_fetch("SELECT transid FROM " . tablename('xc_lv_order') . " WHERE id = :id", array(
                        ':id' => $id
                    ));
                    if (!empty($item['transid'])) {
                        $this->changeWechatSend($id, 1);
                    }
                    pdo_update('xc_lv_order', array(
                        'status' => 2,
                        'remark' => $_GPC['remark'],
                ...
                if (checksubmit('cancelorder')) {
                    if ($item['status'] == 1) {
                        ...
                        mc_credit_update($memberId, 'credit2', $item['price'], array(
                            $_W['uid'],
                            '微商城取消订单退款说明'
                        ));
                    }
                ...
                $dispatch = pdo_fetch("SELECT * FROM " . tablename('xc_lv_dispatch') . " WHERE id = :id", array(
                    ':id' => $item['dispatch']
                ));
                if (!empty($dispatch) && !empty($dispatch['express'])) {
                    $express = pdo_fetch("select * from " . tablename('xc_lv_express') . " WHERE id=:id limit 1", array(
                        ":id" => $dispatch['express']
                    ));
                }
                $item['user']  = explode('|', $item['address']);
                $goods         = pdo_fetchall("SELECT g.*, o.total,o.ettotal,g.type,o.optionname,o.optionid,o.price as orderprice FROM " . tablename('xc_lv_order_goods') . " o left join " . tablename('xc_lv_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid='{$id}'");
                $item['goods'] = $goods;
            } elseif ($operation == 'delete') {
                $orderid = intval($_GPC['id']);
                if (pdo_delete('xc_lv_order', array(
                    'id' => $orderid
                ))) {
                    ...
                    message('订单不存在或已被删除', $this->createWebUrl('order', array(
                        'op' => 'display'
                    )), 'error');
                }
            }
            ...
            if ($operation == 'display') {
                ...
                $item['user']  = explode('|', $item['address']);
                $goods         = pdo_fetchall("SELECT g.*, o.total,g.type,o.optionname,o.optionid,o.price as orderprice FROM " . tablename('xc_lv_order_goods') . " o left join " . tablename('xc_lv_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid='{$id}'");
                $item['goods'] = $goods;
            } elseif ($operation == 'delete') {
                $orderid = intval($_GPC['id']);
                if (pdo_delete('xc_lv_order', array(
    ...
    {
        $goods = pdo_fetchall("SELECT g.id, g.title, g.thumb, g.unit, g.marketprice,g.total as goodstotal,o.total,o.optionid,g.sales FROM " . tablename('xc_lv_order_goods') . " o left join " . tablename('xc_lv_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid='{$id}'");
        foreach ($goods as $item) {
            if ($minus) {
                if (!empty($item['optionid'])) {
                    pdo_query("update " . tablename('xc_lv_goods_option') . " set stock=stock-:stock where id=:id", array(
                        ":stock" => $item['total'],
                        ":id" => $item['optionid']
                    ));
                }
    ...
    public function doMobilelist()
    {
        ...
        if (!empty($_GPC['ccate'])) {
            ...
            $_GPC['pcate'] = pdo_fetchcolumn("SELECT parentid FROM " . tablename('xc_lv_category') . " WHERE id = :id", array(
        ...
        foreach ($category as &$c) {
            if ($c['isrecommand'] == 1) {
                ...
                $c['pager']          = pagination($c['total'], $pindex, $psize, $url = '', $context = array(
                    'before' => 0,
                    'after' => 0,
                    'ajaxcallback' => ''
                ));
            ...
            if (!empty($children[$c['id']])) {
                foreach ($children[$c['id']] as &$child) {
                    if ($child['isrecommand'] == 1) {
                        ...
                        $child['pager']      = pagination($child['total'], $pindex, $psize, $url = '', $context = array(
                            'before' => 0,
                            'after' => 0,
                            'ajaxcallback' => ''
                        ));
    ...
    public function doMobilelistmore()
    {
        ...
        $sql                = 'SELECT * FROM ' . tablename('xc_lv_goods') . ' WHERE `weid` = :weid AND `deleted` = :deleted AND `status` = :status ' . $condition . ' ORDER BY `displayorder` DESC, `sales` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
        $params[':deleted'] = 0;
        $params[':status']  = 1;
        $list               = pdo_fetchall($sql, $params);
        include $this->template('list_more');
    }
    public function doMobilenavi_sound()
    {
        global $_GPC, $_W;
        $idd    = $_GPC['id'];
        $goods  = pdo_fetch("SELECT yinpin FROM " . tablename('xc_lv_goods') . " WHERE id = '$idd'");
        $yinpin = json_decode($goods['yinpin']);
        include $this->template('navi_sound');
    }
    ...
    public function doMobilehuodong()
    {
        ...
        $sorturl = $this->createMobileUrl('huodong', array(
            "keyword" => $_GPC['keyword'],
            "pcate" => $_GPC['pcate'],
            "ccate" => $_GPC['ccate']
        ), true);
    ...
    public function doMobileMyCart()
    {
        ...
        if ($op == 'add') {
            ...
            $goods    = pdo_fetch("SELECT id, type, total,marketprice,maxbuy FROM " . tablename('xc_lv_goods') . " WHERE id = :id", array(
                ':id' => $goodsid
            ));
            if (empty($goods)) {
            ...
            if (!empty($optionid)) {
                $option = pdo_fetch("select marketprice from " . tablename('xc_lv_goods_option') . " where id=:id limit 1", array(
                    ":id" => $optionid
                ));
                if (!empty($option)) {
            ...
            $row = pdo_fetch("SELECT id, total FROM " . tablename('xc_lv_cart') . " WHERE from_user = :from_user AND weid = '{$_W['uniacid']}' AND goodsid = :goodsid  and optionid=:optionid", array(
            ...
            if ($row == false) {
                ...
                pdo_update('xc_lv_cart', $data, array(
                    'id' => $row['id']
                ));
            }
            $carttotal = $this->getCartTotal();
            ...
            if (!empty($list)) {
                foreach ($list as &$item) {
                    $goods  = pdo_fetch("SELECT  title, thumb, marketprice, unit, total,maxbuy FROM " . tablename('xc_lv_goods') . " WHERE id=:id limit 1", array(
                        ":id" => $item['goodsid']
                    ));
                    $option = pdo_fetch("select title,marketprice,stock from " . tablename("xc_lv_goods_option") . " where id=:id limit 1", array(
                        ":id" => $item['optionid']
                    ));
                    if ($option) {
    ...
    public function doMobileConfirm()
    {
        ...
        if (!empty($id)) {
            ...
            if (in_array($arr, $av)) {
                ...
                message('不存在该日期和价格', $this->createMobileUrl('detail', array(
                    'id' => $id
                )), 'error');
            }
            if (empty($item)) {
                message('商品不存在或已经下架', $this->createMobileUrl('detail', array(
                    'id' => $id
                )), 'error');
            }
            if ($item['istime'] == 1) {
                if (time() > $item['timeend']) {
                    $backUrl = $this->createMobileUrl('detail', array(
                        'id' => $id
                    ));
            ...
            if (!empty($optionid)) {
                $option = pdo_fetch("select title,marketprice,weight,stock from " . tablename("xc_lv_goods_option") . " where id=:id limit 1", array(
                    ":id" => $optionid
                ));
                if ($option) {
            ...
            $direct     = true;
            $sql        = 'SELECT SUM(`og`.`total`) AS `orderTotal` FROM ' . tablename('xc_lv_order_goods') . ' AS `og` JOIN ' . tablename('xc_lv_order') . ' AS `o` ON `og`.`orderid` = `o`.`id` WHERE `og`.`goodsid` = :goodsid AND `o`.`from_user` = :from_user';
            $params     = array(
                ':goodsid' => $id,
                ':from_user' => $_W['fans']['from_user']
            );
            ...
            if ((($orderTotal + $item['total']) > $item['usermaxbuy']) && (!empty($item['usermaxbuy']))) {
                message('您已经超过购买数量了', $this->createMobileUrl('detail', array(
                    'id' => $id
                )), 'error');
            }
        ...
        if (!$direct) {
            ...
            if (!empty($list)) {
                foreach ($list as &$g) {
                    $item   = pdo_fetch("select id,thumb,title,weight,marketprice,total,type,totalcnf,sales,unit from " . tablename("xc_lv_goods") . " where id=:id limit 1", array(
                        ":id" => $g['goodsid']
                    ));
                    $option = pdo_fetch("select title,marketprice,weight,stock from " . tablename("xc_lv_goods_option") . " where id=:id limit 1", array(
                        ":id" => $g['optionid']
        ...
        if (checksubmit('submit')) {
            $sendtype = 1;
            $address  = pdo_fetch("SELECT * FROM " . tablename('xc_lv_member_address') . " WHERE id = :id", array(
            ...
            $data = array(
                ...
                'remark' => $_GPC['remark'],
                'address' => $address['username'] . '|' . $address['mobile'] . '|' . $address['zipcode'] . '|' . $address['province'] . '|' . $address['city'] . '|' . $address['district'] . '|' . $address['address'],
                'createtime' => TIMESTAMP
            );
            ...
            foreach ($allgoods as $row) {
                ...
                $o = pdo_fetch("select title from " . tablename('xc_lv_goods_option') . " where id=:id limit 1", array(
                    ":id" => $row['optionid']
                ));
                if (!empty($o)) {
            ...
            message('提交订单成功,现在跳转到付款页面...', $this->createMobileUrl('pay', array(
                'orderid' => $orderid
            )), 'success');
        }
        $carttotal = $this->getCartTotal();
        $profile   = fans_search($_W['fans']['from_user'], array(
            'resideprovince',
            'residecity',
            'residedist',
            'address',
        ...
        $row       = pdo_fetch("SELECT * FROM " . tablename('xc_lv_member_address') . " WHERE isdefault = 1 and uid = :uid limit 1", array(
            ':uid' => $_W['fans']['uid']
        ));
    ...
    public function doMobileBook()
    {
        ...
        if (checksubmit('submit')) {
            ...
            message('报名成功', $this->createMobileUrl('myorder', array(
                'status' => 5
            )), 'success');
        }
        $carttotal = $this->getCartTotal();
    ...
    {
        global $_W;
        $order = pdo_fetch("SELECT * FROM " . tablename('xc_lv_order') . " WHERE id = :id limit 1", array(
            ':id' => $orderid
        ));
        if (empty($order)) {
            return false;
        ...
        if (!empty($orderGoods)) {
            ...
            foreach ($orderGoods as $goods) {
                $goodsCredit = pdo_fetchcolumn($sql, array(
                    ':id' => $goods['goodsid']
                ));
        ...
        if (!empty($credit)) {
            ...
            $fans = fans_search($uid, array(
                "credit1"
            ));
            if (!empty($fans)) {
                if (!empty($add)) {
                    mc_credit_update($_W['member']['uid'], 'credit1', $credit, array(
                        '0' => $_W['member']['uid'],
                        '购买商品赠送'
                    ));
                } else {
                    mc_credit_update($_W['member']['uid'], 'credit1', 0 - $credit, array(
                        '0' => $_W['member']['uid'],
                        '微商城操作'
                    ));
                }
    ...
    public function doMobilePay()
    {
        ...
        $order   = pdo_fetch("SELECT * FROM " . tablename('xc_lv_order') . " WHERE id = :id", array(
            ':id' => $orderid
        ));
        if ($order['status'] != '0') {
        ...
        $goodsId           = pdo_fetchcolumn($sql, array(
            ':orderid' => $orderid
        ));
        $sql               = 'SELECT `title` FROM ' . tablename('xc_lv_goods') . " WHERE `id` = :id";
        $goodsTitle        = pdo_fetchcolumn($sql, array(
            ':id' => $goodsId
        ));
        $params['tid']     = $orderid;
    ...
    public function doMobileMyOrder()
    {
        ...
        if ($op == 'confirm') {
            $orderid = intval($_GPC['orderid']);
            $order   = pdo_fetch("SELECT * FROM " . tablename('xc_lv_order') . " WHERE id = :id AND from_user = :from_user", array(
                ':id' => $orderid,
                ':from_user' => $_W['fans']['from_user']
            ));
            if (empty($order)) {
            ...
            $goods   = pdo_fetchall("SELECT g.id, g.title, g.thumb, g.unit, g.marketprice, o.total,o.optionid FROM " . tablename('xc_lv_order_goods') . " o left join " . tablename('xc_lv_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid='{$orderid}'");
            foreach ($goods as &$g) {
                $option = pdo_fetch("select title,marketprice,weight,stock from " . tablename("xc_lv_goods_option") . " where id=:id limit 1", array(
                    ":id" => $g['optionid']
                ));
            ...
            unset($g);
            $dispatch = pdo_fetch("select id,dispatchname from " . tablename('xc_lv_dispatch') . " where id=:id limit 1", array(
                ":id" => $item['dispatch']
            ));
            ...
            if (!empty($list)) {
                foreach ($list as &$row) {
                    ...
                    $goods   = pdo_fetchall("SELECT g.id, g.title, g.thumb, g.unit, g.marketprice,o.ettotal,o.productprice,o.total,o.optionid FROM " . tablename('xc_lv_order_goods') . " o left join " . tablename('xc_lv_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid='{$row['id']}'");
                    foreach ($goods as &$item) {
                        $option = pdo_fetch("select title,marketprice,weight,stock from " . tablename("xc_lv_goods_option") . " where id=:id limit 1", array(
                            ":id" => $item['optionid']
                        ));
                    ...
                    $row['goods']    = $goods;
                    $row['total']    = $goodsid;
                    $row['dispatch'] = pdo_fetch("select id,dispatchname from " . tablename('xc_lv_dispatch') . " where id=:id limit 1", array(
                        ":id" => $row['dispatch']
                    ));
                }
            }
    ...
    public function doMobileDetail()
    {
        ...
        global $_W, $_GPC;
        $goodsid = intval($_GPC['id']);
        $goods   = pdo_fetch("SELECT * FROM " . tablename('xc_lv_goods') . " WHERE id = :id", array(
            ':id' => $goodsid
        ));
        $op      = $_GPC['op'] ? $_GPC['op'] : '';
        if ($op == 'laqu') {
            ...
            $title = $goods['title'];
            pdo_query("update " . tablename('xc_lv_goods') . " set viewcount=viewcount+1 where id=:id and weid='{$_W['uniacid']}' ", array(
                ":id" => $goodsid
            ));
            $piclist1 = array(
                array(
            ...
            foreach ($allspecs as &$s) {
                $s['items'] = pdo_fetchall("select * from " . tablename('xc_lv_spec_item') . " where  `show`=1 and specid=:specid order by displayorder asc", array(
                    ":specid" => $s['id']
                ));
            }
            unset($s);
            $options = pdo_fetchall("select id,title,thumb,marketprice,productprice,costprice, stock,weight,specs from " . tablename('xc_lv_goods_option') . " where goodsid=:id order by id asc", array(
                ':id' => $goodsid
            ));
            $specs   = array();
            if (count($options) > 0) {
            ...
            $params    = pdo_fetchall("SELECT * FROM " . tablename('xc_lv_goods_param') . " WHERE goodsid=:goodsid order by displayorder asc", array(
                ":goodsid" => $goods['id']
            ));
            $carttotal = $this->getCartTotal();
    ...
    public function doMobileLianxi()
    {
        ...
        if ($operation == 'post') {
            ...
            if (!empty($id)) {
                unset($data['uniacid']);
                unset($data['uid']);
                pdo_update('xc_lv_member_address', $data, array(
                    'id' => $id
                ));
                message($id, '', 'ajax');
            } else {
                pdo_update('xc_lv_member_address', array(
                    'isdefault' => 0
                ), array(
                    'uniacid' => $_W['uniacid'],
                    'uid' => $_W['fans']['uid']
            ...
            if (!empty($id)) {
                ...
                $address = pdo_fetch($sql, $params);
                if (!empty($address)) {
                    pdo_delete('xc_lv_member_address', array(
                        'id' => $id
                    ));
                    if ($address['isdefault'] > 0) {
    ...
    public function payResult($params)
    {
        ...
        $data           = array(
            'status' => $params['result'] == 'success' ? 5 : 0
        );
        $paytype        = array(
            'credit' => '1',
            'wechat' => '2',
        ...
        $goods = pdo_fetchall("SELECT `goodsid`, `total` FROM " . tablename('xc_lv_order_goods') . " WHERE `orderid` = :orderid", array(
            ':orderid' => $params['tid']
        ));
        if (!empty($goods)) {
            $row = array();
            foreach ($goods as $row) {
                $goodsInfo   = pdo_fetch("SELECT `total`, `totalcnf`, `sales` FROM " . tablename('xc_lv_goods') . " WHERE `id` = :id", array(
                    ':id' => $row['goodsid']
                ));
                $goodsupdate = array();
                ...
                pdo_update('xc_lv_goods', $goodsupdate, array(
                    'id' => $row['goodsid']
                ));
            }
        }
        pdo_update('xc_lv_order', $data, array(
            'id' => $params['tid']
        ));
        if ($params['from'] == 'return') {
            ...
            $text      = '用户' . $address[0] . ',电话:' . $address[1] . '于' . date('m月d日H:i') . '成功支付订单' . $order['ordersn'] . ',总金额' . $params['fee'] . '元' . '请耐心等待商家电话联系.祝您旅行快乐';
            $send      = array(
                "touser" => $_W['openid'],
                "msgtype" => "text",
                "text" => array(
    ...
    public function doWebExpress()
    {
        ...
        if ($operation == 'display') {
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($id)) {
                    unset($data['parentid']);
                    pdo_update('xc_lv_express', $data, array(
                        'id' => $id
                    ));
                } else {
            ...
            pdo_delete('xc_lv_express', array(
                'id' => $id
            ));
    ...
    public function doWebDispatch()
    {
        ...
        if ($operation == 'display') {
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($id)) {
                    pdo_update('xc_lv_dispatch', $data, array(
                        'id' => $id
                    ));
                } else {
            ...
            pdo_delete('xc_lv_dispatch', array(
                'id' => $id
            ));
    ...
    public function doWebAdv()
    {
        ...
        if ($operation == 'display') {
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($id)) {
                    pdo_update('xc_lv_adv', $data, array(
                        'id' => $id
                    ));
                } else {
            ...
            $adv = pdo_fetch("select * from " . tablename('xc_lv_adv') . " where id=:id and weid=:weid limit 1", array(
                ":id" => $id,
                ":weid" => $_W['uniacid']
            ));
        } elseif ($operation == 'delete') {
    ...
    public function doMobileOrder()
    {
        ...
        $redirect    = $this->createMobileUrl('myorder', array(
            'status' => $referStatus
        ));
        if (empty($orderId)) {
        ...
        if ($_GPC['op'] == 'delete') {
            ...
            $res  = $acc->sendCustomNotice($send);
            pdo_delete('xc_lv_order', array(
                'id' => $orderId
            ));
            pdo_delete('xc_lv_order_goods', array(
                'orderid' => $orderId
            ));
