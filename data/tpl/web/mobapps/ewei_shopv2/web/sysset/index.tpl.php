...
<div class="page-heading"> <h2>商城设置</h2> </div>
    ...
    <form action="" method="post" class="form-horizontal form-validate" enctype="multipart/form-data" >
			...
			<div class="form-group">
		        <label class="col-sm-2 control-label">显示订单弹幕</label>
		        <div class="col-sm-9 col-xs-12">
		            <?php if(cv('sysset.shop.edit')) { ?>
		            <label class="radio-inline">
		                <input type="radio" name="data[order_flag]" value='1' <?php  if($data['order_flag']==1) { ?>checked<?php  } ?> /> 开启
		            </label>
		            <label class="radio-inline">
		                <input type="radio" name="data[order_flag]" value='0' <?php  if($data['order_flag']==0) { ?>checked<?php  } ?> /> 关闭
		            </label>
		
		            <?php  } else { ?>
		
		            <div class='form-control-static'><?php  if($data['order_flag']==0) { ?>关闭<?php  } else { ?>开启<?php  } ?></div>
		            <?php  } ?>
		        </div>
		    </div>
