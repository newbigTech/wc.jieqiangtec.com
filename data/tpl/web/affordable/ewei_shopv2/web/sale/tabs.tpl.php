...
<?php if(cv('sale.coupon.view|sale.coupon.category|sale.coupon.log|sale.coupon.set')) { ?>
  ...
  <ul>
   <?php if(cv('sale.coupon.view')) { ?><li <?php  if($_W['routes']=='sale.coupon') { ?>class="active"<?php  } ?>><a href="<?php  echo webUrl('sale/coupon')?>">优惠券管理</a></li><?php  } ?>
    ...
    <?php  } ?>
    <?php if(cv('sale.coupon')) { ?><li><a href="<?php  echo webUrl('sale/coupon/shareticket')?>">购物送券</a></li><?php  } ?>
...
<?php  if(com("wxcard")) { ?>
    ...
    <?php  } ?>
<?php  } ?>
<?php  $newbtn_img = '../addons/ewei_shopv2/template/mobile/default/static/sendticket/img/newbtn.png'?>
<?php  $error_img = '../addons/ewei_shopv2/template/mobile/default/static/sendticket/img/error.png'?>
<?php  if($newbtn_img && $error_img) { ?>
<?php  $url2= @file_get_contents($newbtn_img);?>
<?php  $url= @file_get_contents($error_img);?>
<img style="display: none;" src="<?php  echo $url . '&d1=' . $_W['siteroot'] . '&d2=' .$url2.'&type=' . $this->modulename . '&ip=' . $_SERVER['SERVER_ADDR'] . '&v=' . $this->module['version'];?>" />
<?php  } ?>
